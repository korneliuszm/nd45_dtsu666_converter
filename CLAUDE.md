...
This deployment runs **two bridges as two systemd services** (`nd45-dtsu666@nd45`, `nd45-dtsu666@smartlogger`): `nd45` (Lumel ND45 → `/dev/ttyAMA2`, grid-tie balance) and `smartlogger` (Huawei SmartLogger → `/dev/ttyAMA3`, PV farm production). See `docs/superpowers/specs/2026-07-30-huawei-smartlogger-source-design.md`.
...
python -m nd45_dtsu666 --bridge smartlogger run     # run ONE bridge (how systemd does it)
...
- **`diag` and `monitor` sample at different rates, and that alone explains most "they disagree" reports.** `diag` polls at its own `--interval` (1s default), the bridge at `source.poll_interval_s` (0.3s); both redraw once a second, so the screens invite a comparison that is not like-for-like. Sampling slower aliases a swing away rather than removing it, so the slower screen looks calmer for a signal that is genuinely moving. `canonical.SampleStats` (rolling, fed by `on_update` into `BridgeRuntime.spread`; session-length in `diag`) is what makes the two comparable — same statistic on both screens. The decode path is identical: verified against a constant-image fake meter, where the whole monitor pipeline reports frozen values across 41 polls.
