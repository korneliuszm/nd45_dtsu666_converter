...
def test_a_metrics_port_colliding_with_any_bridge_is_rejected():
    scrape_port = load_config("config/config.json").prometheus.port
    raw = _with_bridges(
        {**SECOND_BRIDGE,
         "dtsu": {"transport": "tcp", "slave_id": 11,
                  "tcp": {"host": "0.0.0.0", "port": scrape_port}}}
    )
...
def test_two_bridges_may_not_share_a_metrics_port():
    ...
    taken = load_config("config/config.json").bridges[0].metrics_port
    raw = _with_bridges({**SECOND_BRIDGE, "metrics_port": taken})
    with pytest.raises(ValidationError, match=f"both use metrics_port {taken}"):
        AppConfig.model_validate(raw)
